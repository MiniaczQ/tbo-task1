...
from project.books.forms import CreateBook, EditBook
...
def create_book():
    ...
    if not form.validate_on_submit():
        return jsonify({"error": f"Error creating book: {form.errors}"}), 400
...
def edit_book(book_id):
    ...
    if not form.validate_on_submit():
        return jsonify({"error": f"Error updating book: {form.errors}"}), 400
    ...
    try:
        ...
        if form.year_published.data is not None:
            book.year_published = form.year_published.data
        if form.book_type.data is not None:
